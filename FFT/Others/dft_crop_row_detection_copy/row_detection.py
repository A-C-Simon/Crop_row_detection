...
# Shortest perpendicular row spacing (px) the auto-estimator will consider.
# Real inter-row periods are tens of pixels; anything shorter is texture
# noise (individual plants, soil clods), not a navigable row pattern.
MIN_PERIOD_PX = 8.0
...
def trace_row_curves(image: np.ndarray, res: RowDetection,
    ...
    # Gate on LOCAL contrast: a windowed-median of the neighbouring slabs.
    # One global median wrongly drops long stretches that are uniformly
    # dimmer (shading, canopy density) yet perfectly trackable; a purely
    # local gate would accept whole neighbourhoods of pure noise. The
    # absolute floor vs the strongest slab keeps the latter out.
    half_w = 5
    loc_med = np.array([np.median(amps[max(0, i - half_w):i + half_w + 1])
                        for i in range(len(amps))])
    good = ((amps >= min_amp_frac * loc_med) &
            (amps >= 0.10 * amps.max()) &
            (covs > 0.30))
...
class RowDetector:
    ...
    def _estimate_spacing(self, mag: np.ndarray, fx: np.ndarray,
        ...
        Periods shorter than MIN_PERIOD_PX are also rejected: such locks are
        plant/soil texture noise, never a navigable row pattern (seen as
        3-8 px "spacing" locks on real forward-looking photos).
        """
        fr = np.sqrt(fx[:, None] ** 2 + fy[None, :] ** 2)
        spec = mag.copy()
        spec[fr < 2.0 / min(*mag.shape)] = 0.0
        spec[fr > 1.0 / MIN_PERIOD_PX] = 0.0
