...
def overlay(det: RowDetector, res, img: np.ndarray, scale: float,
            out_path: str, title: str, traces=None):
    m, n = img.shape
    ...
    for line in det.row_lines(res):
        axes[1].plot([line[1], line[3]], [line[0], line[2]],
                     color="0.55", ls=":", lw=1)
    if traces:
        # NaN gaps inside each polyline break it naturally where rows fade
        for poly in traces:
            axes[1].plot(poly[:, 1], poly[:, 0], "r-", lw=2)
    ...
    if len(d):
        ...
        t = np.linspace(-max(m, n), max(m, n), 2)
...
def main():
    ...
    if args.scan_pitch:
        ...
        for p in np.arange(lo, hi + 1e-9, step):
            try:
                ...
                res_p = d.detect(r)
                conf = pitch_confidence(r, d, res_p)
        ...
        print(f"selected pitch {args.pitch:.1f} deg (confidence {best[0]:.1f})")
    ...
    if args.pitch > 0:
        ...
        scale = args.scale if args.scale is not None else 26.0 / img.shape[0]
