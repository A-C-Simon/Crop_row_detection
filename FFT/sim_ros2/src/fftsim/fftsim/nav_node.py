#!/usr/bin/env python3
"""FFT crop-row navigation node for the Gazebo test rig.

Subscribes to the simulated rover's front camera (/camera/image_raw), runs
the DFT detector (FFT/dft_crop_row_detector.py) plus the shared visual
servoing (LinReg/MultiROI/mr_vs.py), and publishes /cmd_vel to the gazebo
diff-drive plugin - closed loop.

Also:
  - subscribes /odom to log world pose / cross-track error (meters of
    lateral deviation; radial error on ring fields),
  - publishes the drawn overlay on /fftsim/overlay (rgb8),
  - writes a CSV + overlay PNGs into log_dir ("" disables).

Usage (after colcon build + source):
  ros2 run fftsim fft_nav --ros-args -p log_dir:=/tmp/fftsim_log
  or see launch/farm.launch.py.
Probe mode (camera/calibration only, no control):
  ros2 run fftsim fft_probe --ros-args -p out_dir:=/tmp -p frames:=5
"""
import csv
import math
import os
...
def _find_multiroi_dir():
    ...
    # MULTIROI_DIR may arrive as a pathsep-joined list (launch appends);
    # try each entry.
    for cand in env.split(os.pathsep):
        if cand and (Path(cand) / "run_mr_navigation.py").exists():
...
# FFT pipeline: DFT detector + shared visual servo
...
class FFTNavNode(Node):
    def __init__(self):
        super().__init__("fft_nav")
        self.bridge = CvBridge()

        # configuration: env vars when launched from farm.launch.py, with
        # ros parameters as fallback (for `ros2 run fftsim fft_nav -p ...`)
        def _p(name, default):
            try:
        ...
        # --- FFT pipeline (DFT detector + shared servo) ---
        self.pipeline = build_pipeline()
        self.get_logger().info(f"algorithm: {self.pipeline.name} "
                               f"(vf_des={self.v_max})")
        # mr_vs vf_des == v_max: cap the shared servo at the rover speed
        if hasattr(self.pipeline, "vs"):
        ...
        self.ovl_pub = self.create_publisher(Image, "/fftsim/overlay", 5)
...
def probe_main(args=None):
    """Camera calibration probe: capture N frames, run DFT detector, save PNGs."""
    rclpy.init(args=args)
    node = rclpy.create_node("fft_probe")
    ...
    # probe uses the FFT pipeline (detector-level calibration view)
    ...
    def cb(msg: Image):
        ...
        try:
            ...
            node.get_logger().info(
                f"frame {got['n']}: n_rows={info.get('n_two_sided')} "
                f"ey={info.get('ey_m', float('nan')):+.2f}m "
                f"theta={info.get('filt_err_theta_deg', float('nan')):+.1f}deg "
                f"prom={info.get('prominence', 0):.1f} status={info.get('status', '')}")
            ovl = out.get("overlay")
            if ovl is not None:
                cv2.imwrite(str(outdir / f"probe_ovl_{got['n']:03d}.png"), ovl)
        except Exception as e:
            node.get_logger().error(f"detect: {e}")
            return
        got["n"] += 1
        if got["n"] >= nframes:
