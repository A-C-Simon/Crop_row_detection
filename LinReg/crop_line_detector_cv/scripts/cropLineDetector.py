...
class cropLineDetector():
    ...
    def __init__(self, original_frame, poly_deg=1, viz_options=NO_VIZ, display=True):
        """
        ...
        @param display       : When False, never open GUI windows (headless/batch mode);
                               the final overlay is still available via self.last_frame
        """
        ...
        # GUI display switch (headless batch runs should set this to False)
        self._display = display

        # Last processed frame with the crop lines overlay (always kept, display or not)
    ...
    def get_heading_angle_error(self, current_frame):
        ...
        fits = self._get_crop_line_indicies_sliding_window(warped_img, left_peak, right_peak, poly_degree=self._poly_degree)
        if fits is None:
            # No usable crop line pixels in this frame: keep the last known angle
            self.last_frame = copy_of_current_frame
    ...
    def _get_crop_line_indicies_sliding_window(self, image, left_peak, right_peak, poly_degree=1):
        ...
        # Not enough points to fit a polynomial (empty first frame or empty
        # history): report failure so the caller can skip this frame
        if len(left_x) < poly_degree + 1 or len(right_x) < poly_degree + 1:
            return None
        ...
        if self._viz_mask & DRAW_WARPED_LANES:
            ...
            # NOTE: the warped image holds 0/255 values, multiplying a uint8
            # by 255 overflows (wraps mod 256); convert to BGR instead
            ...
            if self._display:
                cv2.imshow("Warped & Colored Slides", out_img)
    ...
    def _get_crop_line_previous_window(self, warped_image, left_fit, right_fit, poly_degree=1):
        ...
        # Not enough points to fit a polynomial (empty frame): skip the update
        if len(left_x) == 0 or len(left_y) == 0 or len(right_x) == 0 or len(right_y) == 0:
            return
        ...
        if self._viz_mask & DRAW_SLIDING_WINDOW_RESULT:
            ...
            # NOTE: uint8 * 255 overflows (wraps mod 256); convert to BGR instead
            out_img = cv2.cvtColor(warped_image, cv2.COLOR_GRAY2BGR)
    ...
    def _get_heading_error(self, bin_crop_line_img):
        ...
        if left_point[0] and left_point[1] and right_point[0] and right_point[1]:
            ...
            # Degenerate geometry (points coincide): keep the previous angle
            if length == 0:
                return
            
            # Calculate the angle between the two lines ( cos(theta) = adj/hyp --> theta = acos(adj/hyp) )
            # Clamp the ratio into acos' domain: float rounding can push it just outside [-1, 1]
            cos_angel = float(math.dist(center_point, heading_arrow_point)
                            /math.dist(center_point, goal_arrow_point))
            self._error_angle = np.arccos(min(1.0, max(-1.0, cos_angel)))
    ...
    def _overlay_crop_lines(self, crop_lines_mask_img, original_img):
        ...
        # BUGFIX: this checked the raw flag value (always truthy) instead of
        # testing it against the user's viz options mask
        if self._viz_mask & DRAW_ANGLE_ERROR_ON_IMAGE:
